    ...
    <div className="space-y-6">
      ...
      <FilterBar>
        <form className="grid gap-2 sm:grid-cols-[minmax(220px,1fr)_160px_140px_auto]">
          <Input name="search" defaultValue={search} placeholder="Search clients…" aria-label="Search clients" />
          ...
          <Select name="sort" defaultValue={sort} aria-label="Sort clients">
            <option value="name">Sort: Name</option><option value="createdAt">Sort: Created</option>
          </Select>
        ...
        {search || status ? (
          <div className="mt-2 flex items-center gap-2 border-t px-1 pt-2 text-xs">
            ...
            <Link href="/clients" className="hover:text-foreground text-muted-foreground ml-auto inline-flex items-center gap-1"><X className="size-3" /> Clear</Link>
          </div>
        ) : null}
      </FilterBar>
      {clients.length ? (
        <>
          ...
          <div className="overflow-hidden rounded-lg border bg-card md:hidden">
            {clients.map((client) => (
              <Link key={client.id} href={`/clients/${client.id}`} className="flex gap-3 border-b p-4 last:border-0 active:bg-muted">
                <span className="bg-muted text-muted-foreground grid size-9 shrink-0 place-items-center rounded-md"><Building2 className="size-4" /></span>
                <span className="min-w-0 flex-1">
                  <span className="flex items-start justify-between gap-2"><span className="font-medium">{client.name}</span><Badge value={client.status} /></span>
                  <span className="text-muted-foreground mt-1 block truncate text-xs">{client.company ?? "No company"}</span>
                  <span className="text-muted-foreground mt-2 flex items-center justify-between gap-3 text-xs"><span className="truncate">{client.email ?? "Email not provided"}</span><span className="shrink-0">{client._count.projects} projects</span></span>
                </span>
              </Link>
            ))}
          </div>
